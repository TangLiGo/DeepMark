    ...
    while True:
        ...
        if detect_flag:
            ...
            #    print("pos", [(startX,startY),(endX,endY)], pos.left(), pos.top(), pos.right(), pos.bottom())
                ...
                # if (endX-startX)*(endY-startY)>5000:
                #     frame = cv2.rectangle(frame, (persons[-1][1][0], persons[-1][1][1]),
                #                           (persons[-1][1][2], persons[-1][1][3]), (0, 255, 0), 8)
                # frame = cv2.rectangle(frame, (persons[-1][1][0], persons[-1][1][1]),
                #                       (persons[-1][1][2], persons[-1][1][3]), (0, 255, 0), 8)
            output.write(frame)
            cv2.imwrite('refresher_imgs/' + str(frame_count) + '.png', frame[100:350,350:570])
            cv2.imshow("frame", frame)
            if cv2.waitKey(25) & 0xFF == ord('q'):
                break
...
#genFeatures('D:\TangLi\datasets\FaceForensics++\manipulated_sequences\Deepfakes\c23\\videos\908_207.mp4')#212
#genFeatures('007_wm2.mp4')
#genFeatures('D:\TangLi\datasets\Celeb-DF\Celeb-real/id12_0002.mp4')
genFeatures('D:\TangLi\datasets\Celeb-DF\Celeb-real/id7_0000.mp4')
#genFeatures('D:\TangLi\datasets\Celeb-DF\Celeb-synthesis\id12_0002\id12_id7_0002.mp4')
