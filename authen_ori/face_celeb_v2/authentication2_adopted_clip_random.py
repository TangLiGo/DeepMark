...
def merge_videoclip(crf_distances,fake_distances,insert_len=100):
  ...
  #  print(fake_distances,crf_distances)
    ...
    cut_start = np.random.randint(0, max(0,min(len(crf_distances), len(fake_distances)) - insert_len)+1, size=1)[0]
    frame_count=0
       ...
       # print(len(fake_distances_set[i]))
        for j in range(len(fake_distances_set[i])):
           # insert_len = np.random.randint(30, max(int(len(crf_distances_set[i])/2),31), size=1)[0]
            if np.max(crf_distances_set[i])>0.4:
               print("error a",np.max(crf_distances_set[i]))
            elif np.min(fake_distances_set[i][j])<0.4:
...
def getAuthenPerformance(re_crf_distances_clips,re_fake_distances_clips,frame_ranges,thd,window=50,step=25):
    ...
    for i in range(len(re_fake_distances_clips)):
        ...
        scores_v.append(np.max(clip_scores))
        labels_v.append([1])
...
def explore_clip_detection_vs_window(compression_level,insert_len):
    ...
    for w_i in range(len(windows)):
        if not os.path.exists('data/celebv2_comp' + str(compression_level) + '_insert_len' + str(insert_len) + '_video_clips_random.npy'):
            ...
            np.save('data/celebv2_comp' + str(compression_level) + '_insert_len' + str(insert_len) + '_video_clips_random.npy',
                    [re_crf_distances, re_fake_distances_clips, frame_ranges])
        else:
            re_crf_distances, re_fake_distances_clips, frame_ranges = np.load(
                'data/celebv2_comp' + str(compression_level) + '_insert_len' + str(insert_len) + '_video_clips_random.npy',
                allow_pickle=True)
...
for compression_level in [0,1,2]:
    ...
    for len_i in range(len(insert_lens)):
        ...
        np.save("data/face_partial_detection_comp" + str(compression_level) + "_insert_len"+str(insert_lens[len_i])+"_temp_random.npy", [aucs_v_ave, aucs_clip_ave])
        aucss = np.load("data/face_partial_detection_comp" + str(compression_level) + "_insert_len"+str(insert_lens[len_i])+"_temp_random.npy", allow_pickle=True)
   ...
   #     aucss = np.load("data/face_partial_detection_comp" + str(compression_level) + "_insert_len"+str(insert_lens[len_i])+".npy", allow_pickle=True)
    ...
    plt.savefig("figs/face_partial_fixed_len_detection_comp" + str(compression_level) + "_first_group_random.pdf")
    plt.show()
